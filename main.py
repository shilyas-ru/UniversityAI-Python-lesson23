...
if __name__ == '__main__':
    ...
    # Работаем с колонками, а не кортежами
    with sl.connect(sqlite_db_name) as con:
        # https://proproprogs.ru/modules/metody-fetchall-fetchmany-fetchone-iterdump
        # Иногда более предпочтительным вариантом выходных данных является
        # не кортеж с данными, а словарь, позволяющий обращаться к элементам
        # по именам полей. Для этого после установления соединения с
        # БД следует прописать вот такую строчку:
        #       con.row_factory = sq.Row
        # Теперь, при выполнении программы увидим, что переменная result
        # в цикле ссылается на объект Row, а не кортеж:
        #       <sqlite3.Row object at 0x00000185B226B8B0>
        con.row_factory = sl.Row
        cur = con.cursor()

    # Делаем выборку только книг.
    # Формат выборки: список из кортежей:
    ...
    # result_dict - объединяем авторов в список, если наименование книги совпадает
    result_dict = {}
    for row in result:
        result_dict[row[0]] = result_dict.get(row[0], []) + [f'{row[1]}']
    pprint.pprint(result_dict)
    # Вывод pprint.pprint(result_dict):
    # {'Автоматизация рутинных задач с помощью Python': ['Эл Свейгарт'],
    #  'Алгоритмы неформально. Инструкция для начинающих питонистов': ['Брэдфорд Такфилд'],
    #  'Высокопроизводительные Python-приложения. Практическое руководство по эффективному программированию': ['Горелик Миша',
    #                                                                                                          'Йен Освальд'],
    ...
    #  'Удавы и питоны Уход и содержание': ['Крицкий А.'],
    #  'Учим Python, делая крутые игры': ['Эл Свейгарт'],
    #  'Цель на 360. Управляй судьбой': ['Пелехатый М.М.', 'Спирица Е.']}
